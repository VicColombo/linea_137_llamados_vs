...
\texttt{llamado\_provincia} tenía originalmente 25 niveles y quedó reducida a 6: \texttt{Buenos Aires}, \texttt{C.A.B.A.}, \texttt{Región Norte} (NOA y NEA), \texttt{Región Central} (Cuyo y región Pampeana), \texttt{Región Patagónica}, y \texttt{NS/NC}.    

\texttt{victima\_nacionalidad} tenía originalmente 9 niveles y quedó reducida a 3: \texttt{Argentina}, \texttt{NS/NC}, y \texttt{Otra} (donde entran todas las demás nacionalidaes ya que ninguna superaba individualmente el 1\% de los llamados). 

\texttt{hecho\_lugar} tenía originalmente 17 niveles y quedó reducida a 6: \texttt{NS/NC}, \texttt{Vivienda de la víctima}, \texttt{Vivienda del agresor}, \texttt{Redes sociales}, \texttt{Espacio/transporte público} (subterráneo, tren, colectivo, plaza, descampado, y calle), y \texttt{Otro} (categoría original más residencia turística, obra en construcción, taxi, albergue transitorio, automóvil, comercio,ámbito educativo, y vivienda de un familiar).   


\texttt{llamante\_vinculo} tenía originalmente 16 niveles y quedó reducida a 5: \texttt{Institución} (hospital, comisaría, escuela, defensoría, y otras instituciones), \texttt{Conocido de la víctima} (madre, padre, abuela/o, hermana/o, otro familiar, y otro conocido), \texttt{Víctima}, \texttt{Agresor}, y \texttt{NS/NC}.


Para \texttt{agresor\_vinculo} tomé la agrupación mostrada en la figura \nameref{conocidodesconocido} presentada en \nameref{exploración}, que reduce la cantidad de niveles de 15 a 4, distinguiendo entre \texttt{Conocido familiar}, \texttt{Conocido no familiar}, \texttt{NS/NC}, y \texttt{Desconocido}.


El conjunto de datos resultante de la reducción manual tiene 36 variables, 18 menos que el original (54). En el \nameref{anex} en el cuadro \ref*{tab:transformaciones} se puede ver un resumen de las variables eliminadas, agrupadas, y trasnformadas.
...
Entrené modelos de SVM con dos reducciones de datos distintas: NMDS y manual aplicando distiontos criterios. Además, para cada reducción, utilicé dos variaciones del conjunto de datos en las variables de edad: edades categorizadas y edades numéricas (quitando los datos faltantes).
...
Para entrenar los modelos de SVM en los dos experimentos correspondientes a las dos formas distintas de reducción del conjunto de datos, separé los datos en \textit{train} y \textit{test} y separé los datos faltantes de convive (NSNS) para que sean el conjunto final de datos no vistos.  
