...
En cuanto a la variable de interés \textit{victima\_convive\_agresor}, encontré en el análisis univariado que un 64.4\% no convive, frente al 14.3\% que sí lo hace. El restante 21.19\% que pertenece a respuestas \textit{NS/NC} es el que tengo intenciones de predecir como \textit{SI} o \textit{NO}(ver firgura \Ref{convivencia}).  
...
Exploré también la relación posible entre \textit{victima\_convive\_agresor}, \textit{victima\_edad}, \textit{llamante\_vinculo}, \textit{hecho\_lugar}, \textit{momento\_dia} (HACER UNO EDAD MENOR MOMENTO D´IA CONVIVE )

En la figura \Ref{convivencia_edad} se pueden ver \textit{boxplots} comparativos de \textit{victima\_edad} según cada categoría de \textit{victima\_convive\_agresor}. Se puede observar que a las tres categorías presentan medias y distribuciones similares, ubicando que las víctimas que conviven con su agresor son en promedio más jóvenes que las que no lo hacen

\begin{figure}[H]
    \begin{center}
    \includegraphics[scale=.6]{images/latex_boxplot_convive_edad.png}
    \caption{Distribución de la edad de la víctima según su convivencia o no con el agresor.}
    \label{convivencia_edad}
    \end{center}
    \end{figure}
