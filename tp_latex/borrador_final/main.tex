...
\begin{itemize}
    ...
    \item \texttt{normalized\_stress}: configurado en \texttt{True} calcula el \textit{stress} NMDS.
 \end{itemize}
...
Para entrenar los modelos de SVM, utilicé el método \texttt{svc} de \textit{scikit-learn}\citetext{\citealp{scikit-learn}} y optimianzdo los hiperparámetros:
...
Cabe remarcar que los valores de \textit{stress} obtenidos en ambas configuraciones de NMDS son considerablemente altos: 0.3, cuando que un valor óptimo suele encontrarse alrededor de 0.05. Esto parece indicar que los reordenamientos no representan adecuadamente las distancias originales, los datos parecen estar distribuídos de manera aleatoria.
...
\subsubsection{Ppreprocesamiento con NMDS}

Para los conjuntos de datos A y B, y para todas las variaciones de \texttt{n\_components}, la optimización de hiperparámetros mediante \textit{gridsearch} encontró siempre los mejores resultados con:
...
Los valores de \textit{stress} para los ordenamientos aún en dimensiones \(>2\) resultaron nuevamente altos, siendo el más bajo 0.16 para 7 dimensiones.

La performance de los modelos, que también se igualó en ambos conjuntos de datos y para todos los \texttt{n\_components}, puede verse en el cuadro \Ref{svmperfonmds} a continuación: 
...
Los modelos resultan sumamente afectados por el desbalance de clases y no generalizan bien para la clase minoritaria. Las métricas F1, cobertura, y precisión son buenas para la clase mayoritaria \texttt{NO} (0.90, 1.00, y 0.82 respectivamente), indicando que son modelos capaces de identificar correctamente la mayoría de los puntos de esta clase. Para la clase minoritaria \texttt{SI}, si bien los modelos no comenten errores de clasificación en falsos positivos (de ahí la precisión de 1.0), tampoco pueden identificar correctamente nigún caso perteneciente a la clase, haciendo que  la cobertura, y por lo tanto F1, sean 0.

Las métricas que promedian el rendimiento de ambas clases, F1 macro (0.45) y cobertura macro (0.50), reflejan correctamente el problema del desbalance de las clases. La métrica general de \textit{accuracy} por el contrario resulta engañosa por sí sola, pero su valor alto (0.82) responde al sobreajuste del modelo para la clase mayoritaria.

Finalmente, al aplicar los mejores modelos a los sets de testeo final ciegos para A y B, encontré que identificaban todos los casos como pertenecientes a la clase mayoritaria \texttt{NO}.  


\subsubsection{Preprocesamiento con reducción manual}

Los modelos entrenados con el dataset reducido a mano dieron mejor. 

Para A la optimización de hiperparámetros mediante \textit{gridsearch} encontró  los mejores resultados con:
...
Para B la optimización de hiperparámetros mediante \textit{gridsearch} encontró  los mejores resultados con:
...
Al comparar las proporciones de \texttt{SI} y \texttt{NO} predichas por los mejores modelos en los sets de testeo ciegos con la proporción existente en el \textit{dataset} original, encontré que \dots
a

Proporción de SI en dataset original 0.18269804441498178 Proporción de SI predichos 0.08551010349926072
Proporción de NO en dataset original 0.8173019555850183 Proporción de NO predichos 0.9144898965007393
...
Conclusión Es decir, las distintas dimensiones y las estragtegias con valores faltantes de edad para el reordenamiento de los datos no parecerían afectar este aspecto  del entrenamiento del modelo.

Addressing class imbalance, possibly through resampling methods (e.g., oversampling the minority class or undersampling the majority class) or cost-sensitive learning, could help improve the model's performance for class 1.0.
