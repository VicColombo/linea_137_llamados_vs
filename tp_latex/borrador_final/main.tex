...
Valores de estrés malos en ordenamiento de NMDS, las visualizaciones no aportaron ninguna información, los datos parecen ubicados de manera aleatoria. El preprocesamiento con NMDS para entrenar un modelo SVM no dio buenos resultados, el clasificdor resulta desbalanceado esto parece indicar que los reordenamientos no representan adecuadamente las distancias originales, los datos parecen estar distribuídos de manera aleatoria.
...
El valor bajo del parámetro de regularización \texttt{C} reportado más arriba (0.1) puede estar contribuyendo largamente a la mala performance del modelo, favoreciendo la sobregenralización de la clase mayoritaria en el primer experimento con NMDS. Sin embargo, no es posible afirmar que es el único motivo por el cual esos modelos performan mal, ya que durante el entrenamiento la implementación probó copn otros valotrs de C como scale y supouestamente estos eran los mejores resultados. En el segundo experimento C es un poco más alto para el dataset A (1) y muchísimo más alto para el dataset B (100) y funciona mucho mejor. 
