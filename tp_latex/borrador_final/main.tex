...
\todo[inline]{terimnar de describir NMDS y gower}

 El NMDS se utiliza a menudo como método de ordenamiento para mostrar similitudes y diferencias entre los datos a través de representarlos en espacios de menor dimensión. En particular, este método prioriza preservar en el nuevo espacio de dimensiones reducidas el ordenamiento relativo de las distancias (o diferencias) entre esos puntos en el espacio original. Para realizar el nuevo ordenamiento en dimensiones reducidas,  el algoritmo de NMDS toma una matriz de distancias entre los puntos\todo[inline]{poner expresión matemática de la matriz de distancias y dij} y aplica una transformación monotónica que le permite obtener otra matriz llamada \textit{matriz de disparidades}. En la matriz de disparidades\todo[inline]{poner expresión matemática de la matriz de disparidades}, se preserva el orden relativo de las distancias originales. \todo[inline]{explicar Stress y poner fórmula} \footnote{Para una explicación más detallada del método NMDS, ver \cite{kruskal1964nonmetric}.}. 
...
%\todo[inline]{poner fórmula de la distancia de Gower}. 
...
De la función para MDS de sklearn\footnote{se utiliza una para escalamiento métrico (MDS) y se ajustan los parámetros necesarios para convertirla en escalamiento no-métrico.} me resultaron relevantes los parámetros
...
\todo[inline]{explicar medidas de correlación que tomé entre las variables}
