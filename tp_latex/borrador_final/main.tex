...
La alta dimensionalidad del conjunto de datos y la multiplicidad de niveles de muchas de las variables presenta desafíos para analizar relaciones multivariadas con la variable objetivo; además, causa problemas de procesamiento en el entrenamiento del modelo predictivo con SVM. Apliqué entonces dos métodos para reducir las dimensiones de los datos:
\begin{enumerate}
    \item \textbf{Escalamiento Multidimensional No Métrico (NMDS)}: Este que método reordena los datos en espacios de menor dimensión no solo facilita el entrenamiento de SVM, sino que también permite geerar visualizaciones en 2 dimensiones para buscar agrupamientos de las clases a predecir.
    \item \textbf{Reducción manual de los datos}: Eliminé, agrupé, y transformé las variables basándome en el \nameref{exploración}.
\end{enumerate}

Complementé estas estrategias de reducción con distintos preprocesamientos para tratar con los datos faltantes en las variables de edad, que explico más adelante. Para evaluar los efectos de estos métodos y los distintos preprocesamientos en el rendimiento del modelo SVM, generé múltiples conjuntos de datos con las distintas variaciones.
...
La utilización de NMDS como preprocesamiento para entrenar SVM fue suscitada por los experimentos de \citeauthor{cai2019incorporating}(\citeyear{cai2019incorporating}) en el campo de la microbiología, quienes encuentran resultados moderadamente buenos en \textit{accuracy} aplicando esta metodoología(p.69).

NMDS, un caso particular de Escalamiento Multidimensional (en adelante, MDS, por sus siglas en inglés \textit{Multidimensional Scaling}), es un método de ordenamiento que se suele utilizar para mostrar similitudes y diferencias entre los datos reorganizándolos en espacios de menor dimensión preservando el ordenamiento relativo de las distancias originales \citetext{\citealp[p. 218]{aid}}.

Utilicé el método \texttt{mds} de la librería Sklearn ajustando los siguientes parámetros:
...
En segundo lugar, agrupé algunos de los restantes en tres nuevas variables por dominio, como se puede ver en el cuadro \Ref{tablaagrup} a continuación.
...
\begin{itemize}
    ...
    \item \texttt{victima\_nacionalidad} tenía originalmente 9 niveles y quedó reducida a 3: \texttt{Argentina}, \texttt{NS/NC}, y \texttt{Otra}. %(donde entran todas las demás nacionalidaes ya que ninguna superaba individualmente el 1\% de los llamados).
    \item \texttt{hecho\_lugar} tenía originalmente 17 niveles y quedó reducida a 6: \texttt{NS/NC}, \texttt{Vivienda de la víctima}, \texttt{Vivienda del agresor}, \texttt{Redes sociales}, \texttt{Espacio/transporte público}, %(subterráneo, tren, colectivo, plaza, descampado, y calle), 
    ...
    \item \texttt{llamante\_vinculo} tenía originalmente 16 niveles y quedó reducida a 5: \texttt{Institución}, %(hospital, comisaría, escuela, defensoría, y otras instituciones), 
...
\item Para \texttt{agresor\_vinculo} tomé la agrupación mostrada en la figura 7, \nameref{conocidodesconocido} presentada en \nameref{exploración}, que reduce la cantidad de niveles de 15 a 4: \texttt{Conocido familiar}, \texttt{Conocido no familiar}, \texttt{NS/NC}, y \texttt{Desconocido}.
\end{itemize}


El conjunto de datos resultante de la reducción manual tiene 36 variables, 18 menos que el original (54). En el cuadro \Ref{tab:transformaciones} del \nameref{anex} se puede ver un resumen de las variables eliminadas, agrupadas, y transformadas.

%\begin{multicols}{2}
 %   \begin{itemize}
  %      \item \texttt{llamado\_fecha\_hora} 
   %     \item \texttt{llamante\_edad}
%        \item \texttt{llamante\_genero}
%        \item \texttt{caso\_judicializado}
 %       \item \texttt{victima\_a\_resguardo}
  %      \item \texttt{victima\_edad}
   %     \item \texttt{victima\_genero}
    %    \item \texttt{victima\_discapacidad} 
     %   \item \texttt{victima\_convive\_agresor}
      %  \item \texttt{llamado\_provincia\_reducida}
       % \item \texttt{victima\_nacionalidad\_reducida}
  %      \item \texttt{hecho\_lugar\_reducida}
  %      \item \texttt{vinculo\_llamante\_reducida}
  %      \item \texttt{agresor\_conocido\_no\_conocido}
  %      \item \texttt{fin\_de\_semana}
  %      \item \texttt{momento\_dia} 
  %      \item \texttt{estacion\_del\_año}
  %      \item Variables de violencia sexual: 12
  %      \item Variables de otros formas de violencia: 7
  %  \end{itemize}
  %  \end{multicols} 


   % \texttt{vs\_tocamiento\_sexual}, \texttt{vs\_intento\_tocamiento}, \texttt{vs\_grooming} 'vs\_exhibicionismo', 'vs\_obligacion\_sacarse\_fotos\_pornograficas', 'vs\_acoso\_sexual', 'vs\_iniciacion\_sexual\_forzada\_inducida','vs\_otra\_forma\_violencia\_sexual', 'vs\_no\_sabe\_no\_contesta','vs\_explotacion\_sexual\_group' 'vs\_violacion\_group', 'vs\_tentativa\_group','ofv\_sentimiento\_amenaza', 'ofv\_amenaza\_explicita','ofv\_violencia\_fisica', 'ofv\_enganio\_seduccion', 'ofv\_grooming','ofv\_otra\_forma\_violencia', 'ofv\_no\_sabe\_no\_contesta', 
...
\begin{table}[]
    ...
    \multirow{2}{*}{\begin{tabular}[c]{@{}c@{}}Reducción con \\ NMDS\end{tabular}}                  & A                & \begin{tabular}[c]{@{}c@{}}Variables de edad transformadas a categóricas. \\ Datos faltantes codificados como \texttt{NS/NC}\end{tabular}                                                       \\ \cline{2-3} 
                                                                                                    & B                & \begin{tabular}[c]{@{}c@{}}Solo datos completos de edad de la víctima (numérica). \\ Edad de quien llama eliminada.\end{tabular}                                                      \\ \hline
    \multirow{2}{*}{\begin{tabular}[c]{@{}c@{}}Reducción manual \\ con métodos mixtos\end{tabular}} & A                & \begin{tabular}[c]{@{}c@{}}Variables de edad transformadas a categóricas. \\ Datos faltantes codificados como \texttt{NS/NC}. \\ Aplicación de one hot encoder pot categorización.\end{tabular} \\ \cline{2-3} 
                                                                                                    & B                & \begin{tabular}[c]{@{}c@{}}Solo datos completos de edad de la víctima (numérica). \\ Edad de quien llama eliminada.\end{tabular}                                                      \\ \hline
    \end{tabular}
    \end{table}    
...
Para el entrenamiento de los modelos de SVM con los \textit{datasets} A y B: 


\begin{enumerate}
    \item Separé \texttt{victima\_convive\_agresor} (\textit{y}) del resto de los datos \textit{X}.
    ...
    \item Generé distintas reducciones NMDS varaindo el parámetro \texttt{n\_components}.
    ...
    \item Dividí \textit{X} e \textit{y} en entrenamiento (80\%) y testeo (\%20) de manera estratificada\footnote{Utilizando el método \texttt{StratifiedShuffleSplit} de sklearn.} para mitigar el desbalance de las clases \texttt{SI}  \texttt{NO}.\footnote{Ver nuevamente la figura 8, \nameref{convivencia}.} 
    \item Con cada variación de reducción por NMDS correspondiente a los distintos \texttt{n\_components}, realicé una optimización de los hiperparámetros de SVM \texttt{kernel}, \texttt{C}, y \texttt{gamma} entrenando con validación cruzada de 5 divisiones. Medí también el \texttt{stress} para cada reducción.
    \item Con los mejores hiperparámentros por f1\_weighted entrené un modelo que apliqué al conjunto de \textit{test} final no visto. Ante la imposibilidad de evaluar el resultado del mejor modelo sobre el test final con métodos tradicionales, medí la proporción de SI NO en el dataset original y en la predicción para compararlas y así estimar la performance. Usé F1 porque hay desbalance de clases y porque me interesa saber no solo accuracy sino también cobertura del modelo.
...
\begin{enumerate}
    \item Encodeé con un método ordinal (ordinal encoder sklearn) las variables \texttt{llamado\_fecha\_hora}, \texttt{momento\_dia}, \texttt{estacion\_del\_año} y ambas variables de edad previamente categorizadas (esto último solo en el caso del \textit{dataset} A, en el caso del B, solo me limité a escalar la edad de la víctima). La elección del tipo de \textit{encoder} ordinal fue para preservar la naturaleza ordinal de las variables al ser transformadas. 
    \item Escalé las variables encodeadas con \textit{ordinal encoder}, siguiendo las recomendaciones de \citealt*{udilua2023encoding}. \todo[inline]{agregar página}
    \item Encodeé las variables cuyas categorías eran solamente \texttt{SI}, \texttt{NO}, incluyendo la variable a predecir, mapeando \texttt{SI}= 1, y \texttt{NO}= 0. (En el caso de la variable objetivo, los \texttt{NS/NC} fueron previamente transformados a celdas vacías.) 
    \item Encodeé el resto de las variables con \textit{one hot encoder}.
    \item Creé el test final ciego con las filas del conjunto de datos pertenecientes originalmente a los casos con \texttt{NS/NC} de la varaible objetivo.
    \item 
\end{enumerate}

la preparación de los datos es distinta para cada caso pero el entrenamiento del modelo es el mismo, con cross validation y gridsearch.
