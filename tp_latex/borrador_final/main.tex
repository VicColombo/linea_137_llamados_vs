...
Entrené modelos de SVM con dos las dos reducciones de datos explicadas: NMDS y manual; y por cada una, las dos variaciones del conjunto de datos en las variables de edad: edades categorizadas (A) y edad numérica de la víctima quitando los datos faltantes (B) (ver Cuadro \ref*{modelosSVM}). 
...
\begin{enumerate}
    \item Separé \texttt{victima\_convive\_agresor} (y) del resto de los datos (X).
    \item Extraje de y las filas correspondientes a los casos con respuesta \texttt{NS/NC}.
    \item Generé la matriz de distancias de Gower a partir de X.
    \item Reduje X aplicando NMDS.
    \item Extraje de X reducido las filas originalmente correspondientes a los casos con respuesta \texttt{NS/NC} de y.
    \item Creé el conjunto de \textit{test} final de casos no vistos, compuesto por los datos extraídos en el paso anterior.
    \item Dividí de menera estratificada los datos de X e y en \textit{train} y \textit{test}.
    \item Realicé una búsqueda de los mejor hiperparámetros con el método gridsearch variando el tipo de kernel, el valor de C y gamma.
