...
\begin{enumerate}
    \item \textbf{Escalamiento Multidimensional No Métrico (NMDS)}: Este que método reordena los datos en espacios de menor dimensión no solo facilita el entrenamiento de SVM, sino que también permite generar visualizaciones en 2 dimensiones para buscar agrupamientos de las clases a predecir.
...
La utilización de NMDS como preprocesamiento para entrenar SVM fue suscitada por los experimentos de \citeauthor{cai2019incorporating}\citeyearpar{cai2019incorporating} en el campo de la microbiología, quienes encuentran resultados con \textit{accuracy} moderadamente buena aplicando esta metodología (p.69). \todo[inline]{esto quizás puede ir al principio en la intro o antecedentes}
...
\subsubsection*{Faltantes en edad}

Las implementaciones usadas tanto para el cálculo de la distancia de Gower como para SVM no admiten datos vacíos. Por lo tanto, trabajé con dos estrategias las variables de edad (las únicas con celdas con datos vacíos), generando dos conjuntos de datos alternativos:
\begin{itemize}
    \item Conjunto A: las variables de edad fueron categorizadas\footnote{1 - 11 años = niñez; 12 - 18 años = adolescencia; 19 - 30 años = juventud; 31 - 65 años = adultez, más de 66 años = vejez.}, y los datos faltantes clasificados como \texttt{NS/NC}.
    \item Conjunto B: la variable \texttt{llamante\_edad} es descartada, y también se descartan las filas con datos vacíos en la variable \texttt{victima\_edad}.  
\end{itemize}
...
Apliqué NMDS se aplicó a los conjuntos de datos A y B  variando los valores de \texttt{n\_components}. Generé visualizaciones con las proyecciones resultantes de \(\texttt{n\_components} = 2\) para evaluar la agrupación basada en \texttt{victima\_convive\_agresor}; y entrené modelos de SVM para cada reducción dimensional.
